...
class TestLinuxSpecific:
    ...
    def test_linux_version_boundaries(self, run_command, backup_current_version):
        ...
        with urllib.request.urlopen("https://binaries.soliditylang.org/linux-amd64/list.json") as response:
        ...
        assert result.returncode != 0
        assert "Invalid version - only solc versions above '0.4.0' are available" in result.stdout, \
        ...
        assert result.returncode != 0
        assert f"Invalid version '{latest_release}' is the latest available version" in result.stdout, \
...
class TestMacOSSpecific:
    ...
    def test_macos_version_boundaries(self, run_command, backup_current_version):
        ...
        # Get and test latest version
        with urllib.request.urlopen("https://binaries.soliditylang.org/macosx-amd64/list.json") as response:
            data = json.loads(response.read())
            latest_release = data["latestRelease"]
        ...
        assert result.returncode != 0
        assert "Invalid version - only solc versions above '0.3.6' are available" in result.stdout, \
            f"Did not fail for version too low. Output: {result.stdout}"
...
class TestWindowsSpecific:
    ...
    def test_windows_version_boundaries(self, run_command, backup_current_version):
        ...
        # Get and test latest version
        with urllib.request.urlopen("https://binaries.soliditylang.org/windows-amd64/list.json") as response:
            data = json.loads(response.read())
            latest_release = data["latestRelease"]
        ...
        assert result.returncode != 0
        assert "Invalid version - only solc versions above '0.4.11' are available" in result.stdout, \
            f"Did not fail for version too low. Output: {result.stdout}"
