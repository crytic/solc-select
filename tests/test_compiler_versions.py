...
class TestCompilerVersions:
    ...
    def test_solc_045(self, run_command, test_contracts_dir, backup_current_version):
        ...
        # Switch to 0.4.5
        run_command("solc-select use 0.4.5 > /dev/null 2>&1", check=True)
        
        # Test successful compilation
        result = run_command(
            f"solc {test_contracts_dir}/solc045_success.sol",
            check=False
        )
        ...
        result = run_command(
            f"solc {test_contracts_dir}/solc045_fail_compile.sol",
            check=False
        )
    ...
    def test_solc_050(self, run_command, test_contracts_dir, backup_current_version):
        ...
        # Switch to 0.5.0
        run_command("solc-select use 0.5.0 > /dev/null 2>&1", check=True)
        
        # Test successful compilation
        result = run_command(
            f"solc {test_contracts_dir}/solc050_success.sol",
            check=False
        )
        ...
        # Test expected compilation failure
        result = run_command(
            f"solc {test_contracts_dir}/solc050_fail_compile.sol",
            check=False
        )
        assert result.returncode != 0
        assert "Error: Functions are not allowed to have the same name as the contract." in result.stdout, \
    ...
    def test_solc_060(self, run_command, test_contracts_dir, backup_current_version):
        ...
        # Switch to 0.6.0
        run_command("solc-select use 0.6.0 > /dev/null 2>&1", check=True)
        
        # Test try/catch feature (new in 0.6.0)
        result = run_command(
            f"solc {test_contracts_dir}/solc060_success_trycatch.sol",
            check=False
        )
        ...
        result = run_command(
            f"solc {test_contracts_dir}/solc060_success_receive.sol",
            check=False
        )
    ...
    def test_solc_070(self, run_command, test_contracts_dir, backup_current_version):
        ...
        # Switch to 0.7.0
        run_command("solc-select use 0.7.0 > /dev/null 2>&1", check=True)
        
        # Test deprecated 'now' keyword
        result = run_command(
            f"solc {test_contracts_dir}/solc070_fail_compile.sol",
            check=False
        )
        assert result.returncode != 0
        ...
        # Test successful compilation
        result = run_command(
            f"solc {test_contracts_dir}/solc070_success.sol",
            check=False
        )
    ...
    def test_solc_080(self, run_command, test_contracts_dir, backup_current_version):
        ...
        # Switch to 0.8.0
        run_command("solc-select use 0.8.0 > /dev/null 2>&1", check=True)
        
        # Test successful compilation
        result = run_command(
            f"solc {test_contracts_dir}/solc080_success.sol",
            check=False
        )
        ...
        result = run_command(
            f"solc {test_contracts_dir}/solc080_success_warning.sol",
            check=False
        )
        ...
        # Test expected compilation failure
        result = run_command(
            f"solc {test_contracts_dir}/solc080_fail_compile.sol",
            check=False
        )
        assert result.returncode != 0
...
class TestVersionSwitching:
    ...
    def test_always_install_flag(self, run_command, solc_select_path, backup_current_version):
        ...
        if not str(artifacts_path).endswith(".solc-select/artifacts"):
    ...
    def test_use_without_install(self, run_command, solc_select_path, backup_current_version):
        ...
        run_command("solc-select install 0.8.1 > /dev/null 2>&1", check=False)